...
export async function sendCampaignBatch(
  ...
  // is what makes retries duplicate-free.
  const claimed = await db
    ...
    .where(
      inArray(
        ...
        sql`(SELECT id FROM campaign_recipients WHERE campaign_id = ${campaign.id} AND status = 'pending' ORDER BY created_at LIMIT ${claimCount})`,
      ),
    )
    .returning();
...
// email_events has 10 columns; 8 rows/statement stays under D1's
